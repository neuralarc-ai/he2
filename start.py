...
def print_manual_instructions():
    """Prints instructions for manually starting Helio o1 services."""
    ...
    print("Once all services are running, access Helio o1 at: http://localhost:3000\n")
...
def main():
    ...
    if "--help" in sys.argv:
        ...
        print("Manage Helio o1 services based on your setup method")
        print("\nOptions:")
    ...
    if setup_method == "manual":
        ...
        print("Managing all Helio o1 services with Docker Compose...\n")
        ...
        if is_up:
            action = "stop"
            msg = "🛑 Stop all Helio o1 services? [y/N] "
        else:
            action = "start"
            msg = "⚡ Start all Helio o1 services? [Y/n] "
        ...
        if action == "stop":
            subprocess.run(["docker", "compose", "down"], shell=IS_WINDOWS)
            print(f"\n{Colors.GREEN}✅ All Helio o1 services stopped.{Colors.ENDC}")
        else:
            subprocess.run(["docker", "compose", "up", "-d"], shell=IS_WINDOWS)
            print(f"\n{Colors.GREEN}✅ All Helio o1 services started.{Colors.ENDC}")
            print(f"{Colors.CYAN}🌐 Access Helio o1 at: http://localhost:3000{Colors.ENDC}")
